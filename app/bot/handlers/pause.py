...
async def pause_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    logger.info("bot.command.pause", user_hash=anon_user_id(user.id))
    ...
    ok = await repo.set_paused(user.id, True)
    if not ok:
        await update.message.reply_text("עוד לא נרשמת. שלח /start כדי להתחיל.")
        return

    await update.message.reply_text(
        "⏸️ התראות מושהות. שלח /resume כדי לחדש."
    )
